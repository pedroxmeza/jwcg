...
webControlGrid.prototype.startMov = function (hCol, col, nRows) {
  try {
    ...
    this.ScNrows = nRows;
    this.WidthOrig =
    parseInt (String (document.getElementById (hCol).style.width));
    this.StatusMov = true;
...
webControlGrid.prototype.moveWidth = function () {
  try {
    if (this.StatusMov) {
	     ...
	     var script = "";
	     var columna =
	     this.SelectHColumn.substring (this.SelectHColumn.length - 2,this.SelectHColumn.length - 1);
	     ...
	     if (newWidth > 0) {
	       ...
	       var rowsToDisplay = this.ScNrows;
	       /* 
	       ...
	       */
	       ...
	       for (var i = firstRowInPage; i < rowsToDisplay; i++) {
