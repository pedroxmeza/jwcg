...
function _jwcgBMSearch (patron, texto) {
  try {
    ...
    var sw = (patron.length==0)?false:true;
    var notFound = true;
    ...
    while (sw) {
      ...
      if (posCoincidencia == -1) {
         ...
         if (textoencontrado == patron) {
		      ...
		      tmp = substr.substring (inicio, fin);
          /* 
            si es algo distinto a un numero, entonces
            encontro el indice, por lo tanto se descarta
          */ 
           Result = true;
           for (var n = 0; n < tmp.length && Result == true; n++) {
            Result = ("0123456789".indexOf(tmp.charAt(n)) == -1)?false:true;
           }
           if (Result) { 
              indice += tmp + ";";
              notFound = false;
    ...
    if (notFound){
       alert('No se encontro un resultado para el criterio "'+patron+'"')
    }
    return indice.split (";");
  } catch (e) {
